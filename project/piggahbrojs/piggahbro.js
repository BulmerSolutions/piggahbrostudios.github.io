...
var PB = {
  ...
  getBaseURL: function(url) {
    ...
    for (i = 0; i < url.length; i++) {
      if (this.hits.substr((this.hits.length - 5), 5) == '.com/') {
        return this.hits;
      } else if (this.hits.substr((this.hits.length - 5), 5) == '.net/') {
        return this.hits;
      } else if (this.hits.substr((this.hits.length - 5), 5) == '.org/') {
        return this.hits;
      } else if (this.hits.substr((this.hits.length - 4), 4) == '.io/') {
        return this.hits;
      } else {
        this.hits += url[i];
      }
  ...
  update: [], //PB.scripts, when it is released
  updateTime: null,
  updateLoop: function() {
    this.now = new Date().getTime(),
      this.dt = this.now - (PB.updateTime || this.now);
    PB.updateTime = this.now;
    for (i = 0; i < PB.update.length; i++) {
      PB.update[i](this.dt);
    }
    requestAnimationFrame(PB.updateLoop);
  }
};

//Start up update loop
PB.updateLoop();
